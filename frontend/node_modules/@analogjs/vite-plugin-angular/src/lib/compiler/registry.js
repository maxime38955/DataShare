 ...
 */
export function scanFile(code, fileName) {
    const entries = [];
    ...
    for (const node of program.body) {
        ...
        for (const declarator of varDecl.declarations || []) {
            ...
            if (!allClassRefs || members.length === 0)
                continue;
    ...
    for (const node of program.body) {
        ...
        for (const dec of decorators) {
            ...
            if (!COMPILABLE_DECORATORS.has(decoratorName))
                continue;
            ...
            if (decoratorName === 'NgModule') {
                ...
                for (const member of members) {
                    ...
                    if (init?.type === 'CallExpression') {
                        ...
                        else if (callee?.type === 'StaticMemberExpression' ||
                        ...
                        const inputAliasIndex = calleeName === 'input.required' || calleeName === 'model.required'
                            ? 0
                            : 1;
                        if (calleeName === 'input' || calleeName === 'input.required') {
                            ...
                            const alias = aliasFromArg(calleeName === 'outputFromObservable' ? 1 : 0);
                    ...
                    for (const mdec of memberDecorators) {
                        ...
                        if (!mexpr)
                            continue;
                        const mdecName = mexpr.type === 'CallExpression'
                            ? mexpr.callee?.name
                            : mexpr.type === 'Identifier'
                                ? mexpr.name
                                : undefined;
                        if (mdecName === 'Input') {
                            const alias = mexpr.arguments?.[0]?.type === 'Literal'
